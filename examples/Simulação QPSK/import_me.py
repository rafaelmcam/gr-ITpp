...
def readOutput(path, mini_preamble, header, img_shape, findHeaderIdx = 0):
    ...
    rcv_array = findHeader(info, header, findHeaderIdx)
...
def findHeader(info, header, findHeaderIdx):
    corr = np.correlate(np.unpackbits(info), np.unpackbits(header))
    pos = np.argwhere(corr == max(corr)).ravel()
    rcv_array = np.packbits(np.roll(np.unpackbits(info), -pos[findHeaderIdx]))
    return rcv_array
